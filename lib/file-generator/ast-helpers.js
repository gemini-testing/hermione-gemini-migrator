...
module.exports.browser = {
    ...
    customWaitForExecute: function(condition, timeout) {
        const arg = t.functionExpression(
            null,
            [],
            t.blockStatement([this.execute(condition, false)])
        );

        return withComment(
            'Wait for JS condition',
            this.waitUntil(arg, timeout)
        );
    },
    waitUntil: function(condition, timeout) {
        const argList = [
            t.isExpression(condition) ? condition : parseExpression(condition)
        ];
...
await browser.touchFlick(${offsets.x}, ${offsets.y}, ${speed}))
    ...
    execute: function(script, isAsync) {
...
function browserActionStatement(name, argList = [], isAsync = true) {
    const wrapper = (isAsync, expression) => {
        return isAsync ?
            t.expressionStatement(t.awaitExpression(expression)) :
