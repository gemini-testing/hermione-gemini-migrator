...
module.exports = function(proxyEmitter, geminiAPI) {
    ...
    proxyEmitter.on(proxyEmitter.events.BEFORE_APPLY, function(proxy, handlerArgs, ctx) {
        ...
        const parrentName = proxy.path[proxy.path.length - 2];
        const targetName = target.name || target._name || proxy.path[proxy.path.length - 1];
        ...
        if (['before', 'after', 'capture'].includes(targetName)) {
            ...
            if (targetName === 'capture' && !_.isObject(name) && thisArg) {
                proxy.rootTarget.ctx.currentStateName = name;

                if (action && _.isFunction(action)) {
                    action._name = '_captureCallback';
                }
            }
        ...
        if (targetName === '_captureCallback') {
            const [actions, find] = argList;

            if (find && _.isFunction(find)) {
                find._name = '_find';
            }
        }
